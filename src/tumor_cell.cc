...
TumorCell::TumorCell(const Real3& position) {
  ...
  SetFluidFraction(
      kDefaultFractionFluidTumorCell);  // Set default fluid fraction
  SetNuclearVolume(
      kDefaultVolumeNucleusTumorCell);  // Set default nuclear volume
  // target volumes
  SetTargetFractionFluid(
  ...
  SetTargetRelationCytoplasmNucleus(
      ...
      (1e-16 + kDefaultVolumeNucleusTumorCell));  // Set target relation between
  ...
  SetTargetCytoplasmSolid(
      (kDefaultVolumeNewTumorCell - kDefaultVolumeNucleusTumorCell) *
      (1 - kDefaultFractionFluidTumorCell));  // Set target cytoplasm solid
                                              // volume to real_t

  SetOncoproteineLevel(SamplePositiveGaussian(
      kOncoproteinMean,
      kOncoproteinStandardDeviation));  // Set initial oncoproteine level with a
                                        // truncated normal distribution
  // SetOncoproteineLevel(1.); //Debug
  oxygen_dgrid_ =
      Simulation::GetActive()->GetResourceManager()->GetDiffusionGrid(
          "oxygen");  // Pointer to oxygen diffusion grid
  immunostimulatory_factor_dgrid_ =
      Simulation::GetActive()->GetResourceManager()->GetDiffusionGrid(
          "immunostimulatory_factor");  // Pointer to immunostimulatory_factor
                                        // diffusion grid
  ...
  older_velocity_ = {
      0, 0,
      0};  // Initialize the velocity of the cell in the previous step to zero

  // Add Consumption and Secretion
  SetOxygenConsumptionRate(
      kDefaultOxygenConsumption);  // Set default oxygen consumption rate
  SetImmunostimulatoryFactorSecretionRate(
      kRateSecretionImmunostimulatoryFactor);  // Set default immunostimulatory
                                               // factor secretion rate
  ComputeConstantsConsumptionSecretion();      // Compute constants for all
                                           // ConsumptionSecretion of Oxygen and
...
void TumorCell::Initialize(const NewAgentEvent& event) {
  ...
  if (auto* mother = dynamic_cast<TumorCell*>(
          event.existing_agent)) {  // if the cell is created from division
    if (event.GetUid() == CellDivisionEvent::kUid) {
      ...
      // diffusion grids
      oxygen_dgrid_ =
          mother->oxygen_dgrid_;  // Pointer to the oxygen diffusion grid
      immunostimulatory_factor_dgrid_ =
          mother->immunostimulatory_factor_dgrid_;  // Pointer to the
                                                    // immunostimulatory_factor
                                                    // diffusion grid
      this->SetOncoproteineLevel(
          mother->oncoproteine_level_);  // inherit oncoproteine level from
                                         // mother cell
      this->SetOxygenConsumptionRate(
          mother->GetOxygenConsumptionRate());  // inherit oxygen consumption
                                                // rate from mother cell
      this->SetImmunostimulatoryFactorSecretionRate(
          mother
              ->GetImmunostimulatoryFactorSecretionRate());  // inherit
                                                             // immunostimulatory
                                                             // factor secretion
                                                             // rate from mother
                                                             // cell
      ...
      real_t new_nuclear_volume = mother->GetNuclearVolume() /
                                  2.0;  // Divide mother's nuclear volume by 2
      ...
      this->SetFluidFraction(
          mother->GetFluidFraction());  // Set fluid fraction to mother's fluid
                                        // fraction
      this->SetOlderVelocity(
          mother->GetOlderVelocity());  // Copy velocity from mother cell
...
void TumorCell::SetTransformationRandomRate() {
  ...
  transformation_random_rate_ =
      1 / (std::max(SamplePositiveGaussian(38.6, 3.7) * 60., 1e-16));
}
...
void StateControlGrowProliferate::Run(Agent* agent) {
  ...
  if (auto* cell = dynamic_cast<TumorCell*>(agent)) {
    ...
    auto* oxygen_dgrid =
        cell->GetOxygenDiffusionGrid();  // Pointer to the oxygen diffusion grid
    ...
    switch (cell->GetState()) {
      case TumorCellState::kAlive: {  // the cell is growing to real_t its size
        ...
        if (ShouldEnterNecrosis(
                oxygen_level,
                cell)) {  // Enter necrosis if oxygen level is too low
        ...
        //  Get the current time step in minutes std::ofstream
        //  file("output/simulation_data_mine" +
        //  std::to_string(current_time/(12*60)) + ".csv", std::ios::app); if
        //  (file.is_open()) { file  << oxygen_level << ","
        ...
        real_t time_to_wait =
            1e100;  // Set a very large time to avoid division by zero
        ...
        if (time_to_wait <
            cell->GetTimerState()) {  // If the timer_state exceeds the time to
                                      // transition, change state (this is a
      ...
      case TumorCellState::kNecroticSwelling: {  // the cell is swelling before
        ...
        if (cell->GetVolume() >=
            2 * kDefaultVolumeNewTumorCell) {  // If the cell has swollen to 2
                                               // times its original volume, it
                                               // lyses
          cell->SetState(TumorCellState::kNecroticLysed);  // Change state to
                                                           // necrotic lysed
          cell->SetTimerState(0);                          // Reset timer_state
      ...
      case TumorCellState::kApoptotic: {
        ...
        //      if (auto* c = dynamic_cast<Consumption*>(beh))
        //      {c->SetQuantity(c->GetQuantity()*kReductionConsumptionDeadCells);}//
        //      Reduce consumption rate else if (auto* s =
        //      dynamic_cast<SecretionWithSaturation*>(beh))
        //      {s->SetQuantity(0);} // Stop secretion of immunostimulatory
        //      factor
        //    }
...
bool StateControlGrowProliferate::ShouldEnterNecrosis(real_t oxygen_level,
  ...
  if (oxygen_level <
      kOxygenLimitForNecrosis) {  // oxygen threshold for considering necrosis
    multiplier = (kOxygenLimitForNecrosis - oxygen_level) /
                 (kOxygenLimitForNecrosis - kOxygenLimitForNecrosisMaximum);
  }
  if (oxygen_level < kOxygenLimitForNecrosisMaximum) {  // threshold for maximum
                                                        // necrosis probability
    multiplier = 1.0;
  }
