...
AnalyzeTumor() {
  ...
  int total_alive_cells = num_tumor_cells_type1 + num_tumor_cells_type2 +
                          num_tumor_cells_type3 + num_tumor_cells_type4;
  real_t average_oncoprotein =
      (total_alive_cells > 0) ? (acumulator_oncoprotein / total_alive_cells)
                              : 0.0;
  real_t average_oxygen_cancer_cells =
...
void OutputSummary::operator()() {
  ...
  if (current_step % frequency_ == 0) {
    ...
    if (file.is_open()) {
      ...
      size_t num_tumor_cells_type1, num_tumor_cells_type2,
          num_tumor_cells_type3, num_tumor_cells_type4,
          num_tumor_cells_type5_dead, num_alive_cart;
      ...
      if (current_step % kStepsOneDay == 0 &&
          kTreatment.find(current_day_int) != kTreatment.end()) {
